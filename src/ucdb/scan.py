...
Expected layout::
    ...
    <root>/<code-id>/<version-or-date>/<document.{pdf,docx,odt,txt,md}>
"""
...
class FoundDocument:
    ...
    def relative_key(self) -> str:
        return f"{self.code_id}/{self.version_label}/{self.path.name}"


def scan_repository(root: Path | str) -> Iterator[FoundDocument]:
    """Yield every supported document beneath *root*.

    Directories that don't follow the ``<code>/<version>/<file>`` convention
    ...
    for code_dir in sorted(p for p in root_path.iterdir() if p.is_dir()):
        for version_dir in sorted(p for p in code_dir.iterdir() if p.is_dir()):
